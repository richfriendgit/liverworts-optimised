...
needed_cols <- c("image","latin_name","family_features","species_features","common_name","habitat","photo")
for (nm in needed_cols) if (!nm %in% names(plants)) plants[[nm]] <- ""

# Helper: turn filenames into full raw URLs (leaves absolute URLs untouched)
build_image_url <- function(base, name_or_url) {
  if (grepl("^https?://", name_or_url)) return(name_or_url)
  paste0(base, URLencode(name_or_url, reserved = TRUE))
}
